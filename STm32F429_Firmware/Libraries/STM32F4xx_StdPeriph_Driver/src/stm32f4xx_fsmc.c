...
@verbatim   
 ...
 [..] The following sequence should be followed to configure the FSMC to interface
   ...
   (#) Then enable the NOR/SRAM Bank, for example:
          FSMC_NORSRAMCmd(FMC_Bank1_NORSRAM2, ENABLE);
...
/**
  ...
  *            @arg FMC_Bank1_NORSRAM1: FSMC Bank1 NOR/SRAM1
  *            @arg FMC_Bank1_NORSRAM2: FSMC Bank1 NOR/SRAM2
  *            @arg FMC_Bank1_NORSRAM3: FSMC Bank1 NOR/SRAM3
  *            @arg FMC_Bank1_NORSRAM4: FSMC Bank1 NOR/SRAM4
  * @retval None
  */
void FSMC_NORSRAMDeInit(uint32_t FSMC_Bank)
{
  ...
  {
    FMC_Bank1->BTCR[FSMC_Bank] = 0x000030DB;
  }
  /* FMC_Bank1_NORSRAM2,  FMC_Bank1_NORSRAM3 or FMC_Bank1_NORSRAM4 */
  else
  {   
    FMC_Bank1->BTCR[FSMC_Bank] = 0x000030D2;
  }
  FMC_Bank1->BTCR[FSMC_Bank + 1] = 0x0FFFFFFF;
  FMC_Bank1E->BWTR[FSMC_Bank] = 0x0FFFFFFF;
}
...
{ 
  ...
  FMC_Bank1->BTCR[FSMC_NORSRAMInitStruct->FSMC_Bank] =
  ...
  {
    FMC_Bank1->BTCR[FSMC_NORSRAMInitStruct->FSMC_Bank] |= (uint32_t)BCR_FACCEN_SET;
  }
  /* Bank1 NOR/SRAM timing register configuration */
  FMC_Bank1->BTCR[FSMC_NORSRAMInitStruct->FSMC_Bank+1] =
  ...
  if(FSMC_NORSRAMInitStruct->FSMC_ExtendedMode == FSMC_ExtendedMode_Enable)
  {
    ...
    FMC_Bank1E->BWTR[FSMC_NORSRAMInitStruct->FSMC_Bank] =
  ...
  {
    FMC_Bank1E->BWTR[FSMC_NORSRAMInitStruct->FSMC_Bank] = 0x0FFFFFFF;
  }
}
...
void FSMC_NORSRAMCmd(uint32_t FSMC_Bank, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    FMC_Bank1->BTCR[FSMC_Bank] |= BCR_MBKEN_SET;
  }
  else
  {
    /* Disable the selected NOR/SRAM Bank by clearing the PBKEN bit in the BCRx register */
    FMC_Bank1->BTCR[FSMC_Bank] &= BCR_MBKEN_RESET;
  }
}
/**
  * @}
...
void FSMC_PCCARDInit(FSMC_PCCARDInitTypeDef* FSMC_PCCARDInitStruct)
{
  ...
  FMC_Bank4->PCR4 = (uint32_t)FSMC_PCCARDInitStruct->FSMC_Waitfeature |
                     FSMC_MemoryDataWidth_16b |  
                     (FSMC_PCCARDInitStruct->FSMC_TCLRSetupTime << 9) |
                     (FSMC_PCCARDInitStruct->FSMC_TARSetupTime << 13);
            
  /* Set PMEM4 register value according to FSMC_CommonSpaceTimingStructure parameters */
  FMC_Bank4->PMEM4 = (uint32_t)FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct->FSMC_SetupTime |
                      (FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct->FSMC_WaitSetupTime << 8) |
                      (FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct->FSMC_HoldSetupTime << 16)|
                      (FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct->FSMC_HiZSetupTime << 24); 
            
  /* Set PATT4 register value according to FSMC_AttributeSpaceTimingStructure parameters */
  FMC_Bank4->PATT4 = (uint32_t)FSMC_PCCARDInitStruct->FSMC_AttributeSpaceTimingStruct->FSMC_SetupTime |
                      (FSMC_PCCARDInitStruct->FSMC_AttributeSpaceTimingStruct->FSMC_WaitSetupTime << 8) |
                      (FSMC_PCCARDInitStruct->FSMC_AttributeSpaceTimingStruct->FSMC_HoldSetupTime << 16)|
                      (FSMC_PCCARDInitStruct->FSMC_AttributeSpaceTimingStruct->FSMC_HiZSetupTime << 24);	
            
  /* Set PIO4 register value according to FSMC_IOSpaceTimingStructure parameters */
  FMC_Bank4->PIO4 = (uint32_t)FSMC_PCCARDInitStruct->FSMC_IOSpaceTimingStruct->FSMC_SetupTime |
...
/**
  ...
  *            @arg FSMC_Bank2_NAND: FSMC Bank2 NAND
  *            @arg FSMC_Bank3_NAND: FSMC Bank3 NAND
  *            @arg FMC_Bank4_PCCARD: FSMC Bank4 PCCARD
...
void FSMC_ITConfig(uint32_t FSMC_Bank, uint32_t FSMC_IT, FunctionalState NewState)
{
  ...
  else
  {
    ...
    if(FSMC_Bank == FSMC_Bank2_NAND)
    {
      ...
      FMC_Bank2->SR2 &= (uint32_t)~FSMC_IT;
    }
    /* Disable the selected FMC_Bank3 interrupts */
    else if (FSMC_Bank == FSMC_Bank3_NAND)
    {
      FMC_Bank3->SR3 &= (uint32_t)~FSMC_IT;
    }
    /* Disable the selected FMC_Bank4 interrupts */
    else
    {
      FMC_Bank4->SR4 &= (uint32_t)~FSMC_IT;
    }
  }
}
...
void FSMC_ClearFlag(uint32_t FSMC_Bank, uint32_t FSMC_FLAG)
{
 /* Check the parameters */
  ...
  if(FSMC_Bank == FSMC_Bank2_NAND)
  {
    FMC_Bank2->SR2 &= ~FSMC_FLAG;
  }  
  else if(FSMC_Bank == FSMC_Bank3_NAND)
  {
    FMC_Bank3->SR3 &= ~FSMC_FLAG;
  }
  /* FMC_Bank4_PCCARD*/
  else
  {
    FMC_Bank4->SR4 &= ~FSMC_FLAG;
  }
}
...
void FSMC_ClearITPendingBit(uint32_t FSMC_Bank, uint32_t FSMC_IT)
{
  ...
  if(FSMC_Bank == FSMC_Bank2_NAND)
  {
    FMC_Bank2->SR2 &= ~(FSMC_IT >> 3);
  }  
  else if(FSMC_Bank == FSMC_Bank3_NAND)
  {
    FMC_Bank3->SR3 &= ~(FSMC_IT >> 3);
  }
  /* FMC_Bank4_PCCARD*/
  else
  {
    FMC_Bank4->SR4 &= ~(FSMC_IT >> 3);
  }
}
