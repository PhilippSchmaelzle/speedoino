...
void I2C::timeOut(uint16_t _timeOut){
	...
	timeOutCount = 320*_timeOut; // 40000 ≃ 125ms --> 320≃1ms
}
...
uint8_t I2C::start(){
	...
	unsigned long loop_counter=timeOutCount;
	TWCR = (1<<TWINT)|(1<<TWSTA)|(1<<TWEN);
	while (!(TWCR & (1<<TWINT))){
		if(!timeOutDelay){continue;}
		if((millis() - startingTime) >= timeOutDelay || !loop_counter){
			lockUp();
			return(1);
		}
		loop_counter--;
...
uint8_t I2C::sendByte(uint8_t i2cData){
	TWDR = i2cData;							// Load Data into TWDR Register
	unsigned long startingTime = millis();
	unsigned long loop_counter=timeOutCount;
